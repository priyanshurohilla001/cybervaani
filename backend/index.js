...
if (!projectId) {
  console.error("Google Cloud Project ID must be set in environment variables.");
  process.exit(1);
}
...
io.on("connection", (socket) => {
  ...
  // Test scam detection - fixed event name and property
  setTimeout(() => {
  ...
  }, 2000); // Increased timeout for better test reliability
...
function scamAlert(scamCategory, probability, socketId, languageCode) {
  const languageData = scamMessagesData.languages.find(lang => lang.code === languageCode);
  if (!languageData) {
  ...
  io.to(socketId).emit("scamdetected", {
    ...
    probability,
    messages: {
      duringCall: scamMessage.duringCall,
      afterCall: scamMessage.afterCall
    }
  });
}
